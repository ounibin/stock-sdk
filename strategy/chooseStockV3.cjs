...
async function main(day1, useLocalData = false) {
  ...
  try {
    ...
    if (useLocalData) {
      ...
      console.log(`正在获取${day1}实时行情数据...`)
      list = await getAllQuotesRealTime(day1)
    ...
    list = list.filter((item) => {
      ...
      const noTooMuchUp = item.changePercent < 7
      return !isKechuang && !isXinSanBan && !isJingA && noTooMuchUp
    })

    const list1 = []
    list.forEach((item) => {
      const item_last = lastList.find((n) => n.code === item.code)
      if (item_last && item.volume > item_last.volume * 1.5 && item_last.open > item_last.close && item.trade === item.high) {
        list1.push(item)
      }
    })

    if (list1.length <= 3) {
      console.log('放量结果不足3只，不进行下一轮筛选')
...
// const today = dayjs().format('20260313')
